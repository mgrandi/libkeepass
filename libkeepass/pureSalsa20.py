...
class Salsa20(object):
    ...
    def encryptBytes(self, data):
        assertstmt = type(data) in (bytes, bytearray)
        assert assertstmt, 'data must be byte string'
        assert self._lastChunk64, 'previous chunk not multiple of 64 bytes'
        lendata = len(data)
        munged = array( 'B', b'\x00' * lendata ) # typecode 'c' was removed in py3, now use 'B' for unsigned char
...
def salsa20_wordtobyte( input, nRounds=20, checkRounds=True ):
    ...
    for i in range( nRounds // 2 ): # use // here for integer division in py3, or just use int(nRounds/2) 
